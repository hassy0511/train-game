"""Sleeping tabby cat — Claude Code version (compare with Codex's `cat-sleep`).

Same cat as `cat-stand-cc` curled up: body lying across X, head at +X resting on the front paws and
facing +Z, eyes closed, striped tail wrapped around the front with its cream tip.
Run: python3 scripts/run-bpy.py assets/blender/cat-sleep-cc.py
"""
...
def main() -> None:
    ...
    add(tail_paint(TAIL), cc.add_swept_tube, "tail", TAIL, [0.030, 0.029, 0.028, 0.027, 0.027, 0.026],
    ...
    k.export(model, NAME, BUDGET, "cat-sleep")
