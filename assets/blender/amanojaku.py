...
def build_hat(solids, purple, gold):
    ...
    k.add_swept_tube(solids, "hat", [tuple(p) for p in path], rad, seg_mats, 14)
    tip = path[-1]
...
def build_cape(solids, purple, gold):
    ...
    cape = k.add_mesh(solids, "cape", verts, faces, [purple, gold], smooth=True)
    k.active(cape)
...
def main() -> None:
    ...
    add(SKIN, k.add_tapered_segment, "neck", (0, 0.875, 0), (0, 0.955, 0), 0.029, 0.027, clay_mat, 14)
    add(torso_paint, k.add_profiled_volume, "torso", [
    ...
    for side in (-1, 1):
        ...
        for i, dx in enumerate((-0.022, 0.0, 0.022)):
            fx = palm[0] + s * dx
            add(SKIN, k.add_swept_tube, f"finger-{side}-{i}",
            ...
            add(SKIN, E, f"fingertip-{side}-{i}", (0.023, 0.022, 0.023), (fx + s * dx * 0.3, 0.470, 0.033),
                clay_mat, 10, 5)
        add(SKIN, k.add_swept_tube, f"thumb-{side}", [(palm[0] - s * 0.030, 0.545, 0.036),
                                                        (palm[0] - s * 0.040, 0.508, 0.046)],
            [0.013, 0.011], [clay_mat], 10)
        add(SKIN, k.add_swept_tube, f"leg-{side}",
        ...
        add(PURPLE, k.add_tapered_segment, f"boot-cuff-{side}", (s * 0.092, 0.135, 0.0), (s * 0.092, 0.222, 0.0),
    ...
    k.add_ellipsoid(solids, "hood", (0.170, 0.095, 0.105), (0, 0.935, -0.058), purple, 16, 8)
    bpy.ops.mesh.primitive_torus_add(major_radius=0.026, minor_radius=0.0085, major_segments=14, minor_segments=6,
                                     location=k.to_blender((0, 0.878, 0.106)), rotation=(math.pi / 2, 0, 0))
