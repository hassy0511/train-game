...
/** Drives from the current stop to station `at` (m): fast, slow at 75 m, brake at 5 m. */
async function driveTo(page: Page, at: number, opts: { whistleAt?: number } = {}): Promise<void> {
  await setNotch(page, 3);
  ...
  await waitForS(page, at - 75);
  await setNotch(page, 1);
  await waitForS(page, at - 5);
